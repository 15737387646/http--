...
class HTTPServer(object):
    ...
    def serve_forever(self):
        ...
        while True:
            ...
            handle_clinet = Thread(target=self.handle_request,args=(connfd,))
            handle_clinet.setDaemon(True)
            handle_clinet.start()
    ...
    def handle_request(self,connfd):
        ...
        try:
            ...
            resopnse = response_handlers + response_body
            connfd.send(response.encode())
            return

        #将请求发给frame得到返回数据结果(响应码和数据)
        status,response_body = \
        self.send_request(env['METHOD'],env['PATH'])    
        
        #根据响应码组织响应头内容
        responseget_headlers = self.get_headlers(status)
